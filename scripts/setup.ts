...
class SetupManager {
	...
	private async collectUserConfig(): Promise<void> {
		...
		console.log('   • Frappe: For Frappe OAuth user login');
		...
		const otherVars = [
			...
			'FRAPPE_OAUTH_AUTHORIZE_URL',
			'FRAPPE_OAUTH_TOKEN_URL',
			'FRAPPE_OAUTH_USERINFO_URL',
			'FRAPPE_OAUTH_SCOPES',
	...
	private static readonly FALLBACK_WORKER_VARS = new Set([
		...
		'FRAPPE_CLIENT_ID', 'FRAPPE_CLIENT_SECRET', 'FRAPPE_BASE_URL', 'FRAPPE_OAUTH_BASE_URL',
		'FRAPPE_OAUTH_AUTHORIZE_URL', 'FRAPPE_OAUTH_TOKEN_URL', 'FRAPPE_OAUTH_USERINFO_URL', 'FRAPPE_OAUTH_SCOPES',
		'FRAPPE_PROJECT_VALIDATE_URL', 'FRAPPE_PROJECT_SYNC_URL', 'FRAPPE_API_KEY', 'FRAPPE_API_SECRET',
		'FRAPPE_BEARER_TOKEN', 'FRAPPE_STRICT_PROJECT_VALIDATION', 'FRAPPE_HTTP_TIMEOUT_MS',
	...
	private displayFinalReport(report: ReadinessReport, resources: ResourceInfo): void {
		...
		const hasOAuth = ['GOOGLE_CLIENT_ID', 'GITHUB_CLIENT_ID', 'FRAPPE_CLIENT_ID', 'GITHUB_EXPORTER_CLIENT_ID'].some(key =>
